...
describe('note', () => {
  ...
  it('creates a note with correct frontmatter', () => {
    const note = createNote(tmpDir, config, 'fleeting', 'My Test Note');
  ...
  it('creates a note with custom body', () => {
    const note = createNote(tmpDir, config, 'fleeting', 'Quick', 'Custom body text\n');
    expect(note.body).toBe('Custom body text\n');
  });

  it('handles slug collision', () => {
    const note1 = createNote(tmpDir, config, 'fleeting', 'Duplicate');
    const note2 = createNote(tmpDir, config, 'fleeting', 'Duplicate');
